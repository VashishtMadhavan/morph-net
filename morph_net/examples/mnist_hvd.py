...
def main(args):
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=args.lr * hvd.size())
    optimizer = hvd.DistributedOptimizer(optimizer)
    train_op = optimizer.minimize(loss_op + reg_loss_op, global_step=global_step)

    hooks = [
        hvd.BroadcastGlobalVariablesHook(0),
        tf.train.StopAtStepHook(last_step=args.steps // hvd.size()),
    ...
    outdir = args.outdir if hvd.rank() == 0 else None

    # Training Loop
    with tf.train.MonitoredTrainingSession(checkpoint_dir=outdir, hooks=hooks, config=config) as mon_sess:
        while not mon_sess.should_stop():
            ...
            if hvd.rank() == 0 and global_step_val % 1000 == 0:
...
def parse_args():
    ...
    parser.add_argument('--outdir', type=str, default='mnist_hvd_debug/', help='where to save checkpoints and structure exports')
